...
local TEMPLE_CENTER_POS = Vector3.new(28282.5703125, 14896.8505859375, 105.1042709350586)
local TEMPLE_ENTRY_POS  = Vector3.new(28310.0234, 14895.1123, 109.456741)
local TOP_OF_GREAT_TREE = CFrame.new(3028, 2281, -7325)
...
function BorrowTempleOfTime()
	...
	if not workspace.Map:FindFirstChild("Temple of Time") then
		local ms = game.ReplicatedStorage:FindFirstChild("MapStash")
		local temple = ms and ms:FindFirstChild("Temple of Time")
		if temple then
...
function IsInTempleOfTime()
	...
	return (hrp.Position - TEMPLE_CENTER_POS).Magnitude < 3000
end
...
function TeleportTempleOfTime()
	...
	if not hrp or not hum or hum.Health <= 0 then return "waiting_character" end

	pcall(function()
		game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", TEMPLE_ENTRY_POS)
	end)
	if IsInTempleOfTime() then return "arrived" end

	pcall(function()
		local v4Status = game.ReplicatedStorage.Remotes.CommF_:InvokeServer("RaceV4Progress", "Check")
		if v4Status == 1 then
	...
	local distToTree = (hrp.Position - TOP_OF_GREAT_TREE.Position).Magnitude
	if distToTree > 30 then
		toTarget(TOP_OF_GREAT_TREE)
...
function PullLeverV4()
	...
	then
		...
		-- Kiểm tra vị trí thực (không dựa vào object) - giống kaiv4_fluent
...
function CheckMultiPlayerNearDoor()
	local _temple = GetTempleOfTime()
	local g, R, m = next, game.Workspace.Characters:GetChildren()
	local l = 0
	for S, I in g, R, m do
		S = GetOtherPlayerRaces()[I.Name]
		local doorPos = _temple and _temple:FindFirstChild(S .. "Corridor")
			and _temple[S .. "Corridor"].Door
			and _temple[S .. "Corridor"].Door.Door
			and _temple[S .. "Corridor"].Door.Door.RightDoor
			and _temple[S .. "Corridor"].Door.Door.RightDoor.Union
			and _temple[S .. "Corridor"].Door.Door.RightDoor.Union.Position
		l = if S
				and doorPos
...
function AutoTrialV4()
	...
	if
		g
			and (g.FFABorder:FindFirstChild("Forcefield"))
			and g.FFABorder.Forcefield.Transparency == 1
	...
	then
		if game:GetService("Players").LocalPlayer.PlayerGui.Main.TopHUDList.RaidTimer.Visible then
			...
			elseif R == "Fishman" then
				local part2 = if game:GetService("Workspace")._WorldOrigin.Locations:FindFirstChild("Trial of Water")
					then (game:GetService("Workspace")._WorldOrigin.Locations:FindFirstChild("Trial of Water"))
					else nil
				if part2 and t:DistanceFromCharacter(part2.Position) < 1500 then
					local m = GetSeaBeastTrial()
					repeat
						task.wait()
						if m and IsMobAlive(m) then
							if Settings["Select Weapon"] == "Blox Fruit" then
								toTarget(m.HumanoidRootPart.CFrame * CFrame.new(-7, 20, 0))
							else
								toTarget(m.HumanoidRootPart.CFrame * CFrame.new(7, 20, 0))
					...
					until not game:GetService("Players").LocalPlayer.PlayerGui.Main.TopHUDList.RaidTimer.Visible
						or t:DistanceFromCharacter(part2.Position) > 1500
				end
			elseif R == "Ghoul" then
				repeat
					task.wait()
	...
	elseif getgenv().VerifyTrial then
		if not Settings["Multi Trial"] then
			...
			if ToggleAutoTrial then ToggleAutoTrial:SetStage(false) end
		end
...
function PlayerTrial()
	local _temple = GetTempleOfTime()
	if not _temple then return nil end
	local g = _temple.FFABorder and _temple.FFABorder.Forcefield
	...
	while task.wait(0.1) do
		pcall(function()
			if Settings["Kill players When complete Trial"] then
				local _t = workspace.Map:FindFirstChild("Temple of Time")
				local _ff = _t and _t:FindFirstChild("FFABorder") and _t.FFABorder:FindFirstChild("Forcefield")
				if _ff and _ff.Transparency ~= 1 then
					if game:GetService("Players").LocalPlayer.PlayerGui.Main.TopHUDList.RaidTimer.Visible then
...
spawn(function()
	while task.wait(0.1) do
		...
		pcall(function()
			...
			local _ff = _t and _t:FindFirstChild("FFABorder") and _t.FFABorder:FindFirstChild("Forcefield")
			if _ff and _ff.Transparency ~= 1 then
				-- Helper bắt buộc reset khi FFA active (không cần toggle)
				t.Character.Humanoid.Health = 0
			end
		end)
	end
